defmodule ElixirLearning.Accounts.Withdraw do

  alias Ecto.Multi

  alias ElixirLearning.{Account, Repo}
  def call(%{"id" => id, "value" => value}) do
    Multi.new()
    |> Multi.run(:account, fn repo, _changes ->  get_account(repo, id)end)
    |> Multi.run(:account, fn repo, %{account: account} -> update_balance(repo, account, value)end)
    |> run_transaction()
  end
  ...
  defp update_account({:error, _reason} = error, __repo, _account), do: error
